...
export function successfulToolCalls(events: readonly SessionEvent[], toolName: string): string[] {
  ...
  for (const event of events) {
    if (event.type === 'tool/call' && event.data.name === toolName) calls.push(event.data.callId)
  }
...
export function foldDisciplineStage(events: readonly SessionEvent[]): DisciplineStage {
  ...
  for (const tool of Object.keys(STAGE_BY_TOOL)) {
    for (const callId of successfulToolCalls(events, tool)) succeeded.add(callId)
  }
  let stage: DisciplineStage = 'grill'
  for (const event of events) {
    if (event.type !== 'tool/call') continue
    const next = STAGE_BY_TOOL[event.data.name]
    if (next === undefined || !succeeded.has(event.data.callId)) continue
    stage = next
  }
  return stage
}
